...
const registerUser = async (req, res) => {
    try {
        const { userType, role, name, email, password, organizationType, team, employeeType, orgAdmins, professionType } = req.body;
        ...
        if(!employeeType){
            const newUser = new User({
                userType, role, name, email, password: hashedPassword, organizationType, team, orgAdmins, professionType: userType === 'Professional' ? professionType : null
            });
            await newUser.save();
            res.status(201).json({
            ...
            const newUser = new User({
                userType, role, name, email, password: hashedPassword, organizationType, team,employeeType, orgAdmins, professionType: userType === 'Professional' ? professionType : null
            });
            await newUser.save();
            res.status(201).json({
                message: "User registered successfully",
